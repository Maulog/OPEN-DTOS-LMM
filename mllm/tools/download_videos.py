# https://github.com/yellow-binary-tree/HawkEye/blob/main/internvid_g/code/download_videos.py
...
def download_video(command):
    ...
    to_execute = f'yt-dlp --proxy socks5://127.0.0.1:10808/ "http://youtu.be/{video_id}" -o "{target_dir}/{video_id}.%(ext)s" -S "+res:720,fps" -q --no-check-certificate'
    res_code = os.system(to_execute) # --proxy socks5://127.0.0.1:10808/
    if res_code:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src_file", type=str, default='train.jsonl', help="folder that contains video ids to download")
    parser.add_argument("--num_workers", type=int, default=5)
    parser.add_argument("--error_file", type=str, default='temp/error_video_ids.txt')
    args = parser.parse_args()

    # 将要加载的视频添加到video_ids集合中
    video_ids = set()
    if args.src_file.endswith('jsonl'):        # download from InternVid annotations or InternVid-G annotations
        for line in open(args.src_file):
            ...
            if 'video' in example:
                video_ids.add(example['video'].split('.')[0])
    ...
    commands = [(video_id, args.target_dir) for video_id in videos_to_download]
