...
def prepare_data(
    ...
    conv_template = conv_args.get('conv_template', 'llama_3')
    conv_template = partial(get_conv_template, name=conv_template) # 不同模型的对话模式
    transforms = conv_args.get('transforms', None) # Expand2square
    if transforms is not None:
        transforms = TRANSFORMS.build(transforms)
    # process func
    process_func = {}
    for k, v in model_args.process_func_args.items():
        process_func[k] = FUNCTIONS.build(cfg=v) # 有conv（转换模板）、target（目标）、text（文本）、image（图像）四种处理函数

    conv_dataset_cls = partial( # WRAPPER_DATASET是SingleImageConvDataset、SingleImageConvSegDataset
        WRAPPER_DATASET.get(data_args.get("dataset_wrapper", "conv"), "conv"),
        preprocessor=preprocessor, # 处理图像
        process_func=process_func, # 包含四种处理函数
        tokenize_kwargs=tokenize_kwargs,
        conv_template=conv_template,
        training_args=training_args, 
        model_args=model_args,
        data_args=data_args,
        transforms=transforms, # Expand2square
    )
    ds = {
...
def prepare_interactive(
        model_args,
        preprocessor: Dict[str, Any],
): # 准备单次推理需要的参数，返回SingleImageInteractive
    conv_args = model_args.conv_args
    tokenize_kwargs = conv_args.get('tokenize_kwargs', {})
