...
def norm_rec(timestamps, clip):
    clip_length = clip[1] - clip[0] 
    timestamps = timestamps / clip_length
    return timestamps

def de_norm_rec(timestamps, clip):
    clip_length = clip[1] - clip[0]
    timestamps = timestamps * clip_length
    return timestamps
...
def norm_box_xyxy(box, *, w, h): # [bs, 4]
    x1, y1, x2, y2 = box[:, 0], box[:, 1], box[:, 2], box[:, 3]
    norm_x1 = torch.clamp(x1 / w, min = 0.0, max = 1.0)
    norm_y1 = torch.clamp(y1 / h, min = 0.0, max = 1.0)
    norm_x2 = torch.clamp(x2 / w, min = 0.0, max = 1.0)
    norm_y2 = torch.clamp(y2 / h, min = 0.0, max = 1.0)
    
    norm_box = torch.stack([norm_x1, norm_y1, norm_x2, norm_y2], dim=-1)
    norm_box = norm_box.view(len(box), 4)
    return norm_box
    
def de_norm_box_xyxy(norm_box, *, w, h): # [bs, 2]
    x1, y1, x2, y2 = norm_box[:, 0], norm_box[:, 1], norm_box[:, 2], norm_box[:, 3]
    x1 = torch.clamp(x1 * w, min=0.0, max=w-1)
    x2 = torch.clamp(x2 * w, min=0.0, max=w-1)
    y1 = torch.clamp(y1 * h, min=0.0, max=h-1)
    y2 = torch.clamp(y2 * h, min=0.0, max=h-1)
    box = torch.stack([x1, y1, x2, y2], dim=-1) # [bs//2, 4]
    box = box.view(len(norm_box), 4)
    return box


def norm_point_xyxy(point, *, w, h): # point [bs, 2]
    x, y = point[:, 0], point[:, 1]
    norm_x = torch.clamp(x / w, min=0.0, max=1.0)
    norm_y = torch.clamp(y / h, min=0.0, max=1.0)
    norm_point = torch.stack([norm_x, norm_y], dim=-1)
    norm_point = norm_point.view(len(point), 2)
    return norm_point

def de_norm_point_xyxy(norm_point, *, w, h):
    x, y = norm_point[:, 0], norm_point[:, 1]
    x = torch.clamp(x * w, min=0.0, max=w-1)
    y = torch.clamp(y * h, min=0.0, max=h-1)
    point = torch.stack([x, y], dim=-1)
    point = point.view(len(norm_point), 2)
    return point
...
def process_images(images, image_processor, model_cfg):
    ...
    if image_aspect_ratio == 'pad':
        for image in images:
            image = expand2square(image, tuple(int(x*255) for x in image_processor.image_mean)) # 将图像扩展为正方形，同时根据image_mean进行标准化
            image = image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0] # clip的预处理
            new_images.append(image)
    else:
        return image_processor(images, return_tensors='pt')['pixel_values']
    if all(x.shape == new_images[0].shape for x in new_images):
        new_images = torch.stack(new_images, dim=0) # 将图像沿dim=0进行堆叠
    return new_images


def box_xyxy_expand2square(box, *, w, h): # （原图）当前图像的宽高，返回修改后的坐标
    if w == h:
        return box
    if w > h:
        x1, y1, x2, y2 = box
...
class Expand2square:
    ...
    def __call__(self, image: Image.Image, labels: Dict[str, Any] = None) -> Tuple[Image.Image, Optional[Dict[str, Any]]]:
        ...
        if 'boxes' in labels: # 将候选框的坐标转换到正方形图像（边长为较长一边）上
            bboxes = [box_xyxy_expand2square(bbox, w=width, h=height) for bbox in labels['boxes']]
            labels['boxes'] = bboxes
        if 'points' in labels: # 单个点的坐标，转换到正方形图像的坐标上
