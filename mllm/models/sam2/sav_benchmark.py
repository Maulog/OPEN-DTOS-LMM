...
class VideoEvaluator:
    ...
    def __call__(self, vid_name: str) -> Tuple[str, Dict[str, float], Dict[str, float]]:
        ...
        to_evaluate, is_sav_format = self.scan_vid_folder(vid_name) # 这里处理单个视频
...
def _seg2bmap(seg, width=None, height=None): # 将分割图变为01
    """
...
class Evaluator:
    def __init__(self, boundary=0.008, name=None, obj_id=None):
        ...
        self.object_iou = defaultdict(list) # 当访问不存在的键会创建一个空列表
    ...
    def feed_frame(self, mask: np.ndarray, gt: np.ndarray):
        ...
        self.objects_in_masks.update(set(mask_objects)) # 这里生成一个列表去保存每个对象的id

        all_objects = self.objects_in_gt.union(self.objects_in_masks) # 得到所有对象的集合，这里看一下变量
    ...
    def conclude(self):
        ...
        for object_id in self.objects_in_gt: # 遍历标签中每个目标
