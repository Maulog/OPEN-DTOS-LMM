# modified from moment_detr/matcher.py
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # targets = targets["span_labels"] # 这里注释
        ...
        if self.span_loss_type == "l1": # 走这里
            # We flatten to compute the cost matrices in a batch
            out_spans = outputs["pred_spans"].flatten(0, 1)  # [batch_size * num_queries, 2]

            # Compute the L1 cost between spans
            cost_span = torch.cdist(out_spans.to(torch.float32), tgt_spans.to(torch.float32), p=1)  # [batch_size * num_queries, total #spans in the batch]
        ...
        sizes = [len(v) for v in targets] # [1,3,2]
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))] # 沿最后一个维度进行分割
