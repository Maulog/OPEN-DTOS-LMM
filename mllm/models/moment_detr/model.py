    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        Parameters:
            ...
            span_loss_type: str, one of [l1, ce]
                ...
                ce: (st_idx, ed_idx) classification. # 注意看这两种loss的区别
        ...
        self.span_embed = MLP(hidden_dim, hidden_dim, span_pred_dim, 3) # 这里输出3个是[st,ed,score]
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, log=True): # 计算置信度分数background_label=1，另一个是0
        ...
        idx = self._get_src_permutation_idx(indices)
        target_classes = torch.full(src_logits.shape[:2], self.background_label, # 将所有生成的query标签设置为背景
                                    dtype=torch.int64, device=src_logits.device)  # (batch_size, #queries)
        target_classes[idx] = self.foreground_label # 将匹配的query标签设置为前景（indices就是匹配的结果）
