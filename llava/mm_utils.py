...
def process_image(image_file, data_args, image_folder): # 先resize再预处理
...
def process_images(images, image_processor, model_cfg):
    ...
    if all(x.shape == new_images[0].shape for x in new_images): # 这里如果为空z
        new_images = torch.stack(new_images, dim=0)
    return new_images


def tokenizer_image_token( # 将原先文本编码，在<image>的token中用IMAGE_TOKEN_INDEX进行替换
