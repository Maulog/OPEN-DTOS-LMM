...
class VisionTower(nn.Module):
    ...
    def feature_select(self, image_forward_outs):
        ...
        if self.select_feature == "clip_patch": # for clip
            image_features = image_features[:, 1:]
        elif self.select_feature == "clip_cls_patch": # for clip
            image_features = image_features
        elif self.select_feature == "siglip_cls_patch": # for siglip
            # pooler_output = image_forward_outs.pooler_output
            # pooler_output = pooler_output.unsqueeze(1)
            # image_features = torch.cat([pooler_output, image_features], dim=1)
            image_features = image_features
        else:
