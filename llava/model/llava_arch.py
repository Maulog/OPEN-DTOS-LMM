...
class LlavaMetaModel(ABC):
    ...
    def encode_images(self, images): # 直接将图像特征传递到映射层
        image_features = self.get_vision_tower()(images)         # [bs, 729, 1152]
        image_features = self.get_mm_projector()(image_features) # [bs, 196(14*14), 4096]
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal( # 该函数一般只进入一次
    ...
    def repack_multimodal_data(
        ...
        # 首先降序排列数据，然后将相似长度的数据组合成一个新的批次，
        # 不需要让较短的数据padding到较长的长度当中，减少padding的操作，
        # 提升计算效率，得到更多的有效信息，减少内存使用。
        # kentang-mit@: reorder and repack (reduce computation overhead)
        # requires transformers replacement.
        new_inputs_embeds = []
        new_position_ids = []
        new_labels = []
        seqlens_in_batch = attention_mask.sum(dim=-1, dtype=torch.int32) # 计算序列长度（除去padding）
        sorted_seqlens_in_batch, sorted_idx = torch.sort(seqlens_in_batch, descending=True)
        # print(sorted_seqlens_in_batch)
        max_seqlen = inputs_embeds.shape[1] # 当前批次中最长的序列长度
        ...
        for i in range(len(sorted_seqlens_in_batch)):
            ...
            if cur_seqlen + cur_batch_len <= max_seqlen: # 当前批次不满，将数据合并到一个批次中
        ...
        new_inputs_embeds = torch.nn.utils.rnn.pad_sequence(
            new_inputs_embeds, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
