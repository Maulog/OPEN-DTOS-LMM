...
class DownSampleBlock(nn.Module):
    ...
    def forward(self, x):
        vit_embeds = x # [6, 729(27*27), 1152] 经过flat_square后变成28*28，总数相等
        h = w = int(vit_embeds.shape[1] ** 0.5)
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], h, w, -1)
        vit_embeds = self.flat_square(vit_embeds)
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], -1, vit_embeds.shape[-1]) # [6, 196(14*14), 4608]
        return vit_embeds

    def flat_square(self, x): # 二维平面下进行下采样
        n, w, h, c = x.size()
        if w % 2 == 1: # 不是偶数拼接0向量
        ...
        x = x.view(n, int(h / 2), int(w / 2), int(c * 4)) # 不一步实现是因为要保证contiguous
        return x
